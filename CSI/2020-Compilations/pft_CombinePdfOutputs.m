...
A = fullfile(Root, ScannerID, sprintf('%s - %s - CSI Matrix Plot.pdf', ScannerID, SubjectID));
B = fullfile(Root, ScannerID, sprintf('%s - %s - Septal Spectra.pdf', ScannerID, SubjectID));
C = fullfile(Root, ScannerID, sprintf('%s - %s - Averaged Septal Spectrum.pdf', ScannerID, SubjectID));

Inputs = {};

if (exist(A, 'file') == 2)
  Inputs = horzcat(Inputs, A);
end

if (exist(B, 'file') == 2)
  Inputs = horzcat(Inputs, B);
end

if (exist(C, 'file') == 2)
  Inputs = horzcat(Inputs, C);
end
